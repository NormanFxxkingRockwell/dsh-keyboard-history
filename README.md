...
| `↑`（空草稿） | 召回最新一条已发送消息 |
| `↑`（翻阅中） | 向更早的消息回翻 |
| `↑`（已到窗口最旧一条） | 自动补载更早的历史窗口（会话日志按需分页），持续按 ↑ 可翻完整段会话 |
| `↓`（翻阅中） | 向更新的消息前进 |
| `↓`（越过最新一条） | 清空草稿，退出翻阅 |
| 编辑 / 回车发送 | 自动退出翻阅态 |

历史直接取自会话快照里的用户消息节点（相邻重复折叠、空消息跳过）；
刷新页面或恢复会话后历史自然重建，无需额外存储。会话日志以「窗口」按需
加载（`loadOlder` 分页），翻到窗口最旧一条时本插件会自动请求更早窗口并按
文本重新锚定位置，因此 ↑ 能一直翻到会话开头——但**被压缩（compaction）
替换为摘要的旧轮次**只剩摘要文本，原始逐条消息在浏览器端不可恢复。
...
编辑退出翻阅、宿主回显规范化（尾随空白）不打断翻阅、最老一条时首个 ↓
立即前进、以及窗口分页（↑ 到最旧一条自动 `loadOlder` 补载 + 文本重锚定）。
