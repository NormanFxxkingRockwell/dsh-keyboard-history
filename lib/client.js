...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function HistoryRecall(props) {
			...
			var loadHistoryOlder = props.loadHistoryOlder;
			var nodes = useSession(function (s) { return s.nodes; });
			/** 更早的历史是否仍在窗口外未加载（true 时 ↑ 到最旧一条可触发补载）。 */
			var hasMore = useSession(function (s) { return s.hasMore === true; });
			...
			/** 草稿偏离召回状态时退出翻阅：清空（发送/手动清除）或真实编辑都退出；
			 * 仅尾随空白差异（宿主对 setDraft 的回显规范化）不视为偏离。 */
			react.useEffect(function () {
				if (!browsingRef.current) return;
				var draft = input.draft;
				if (draft === lastSetRef.current) return;
				if (draft === "") {
					resetBrowse();
					return;
				}
				var expected = lastSetRef.current;
				if (expected !== null && draft.replace(/\s+$/, "") === expected) return;
				resetBrowse();
			}, [input.draft]);
			/** 窗口补载（更早历史前插）后，按文本把当前位置重新锚定；条目被压缩/移除则退出翻阅。 */
			react.useEffect(function () {
				if (!browsingRef.current || lastSetRef.current === null) return;
				var idx = history.lastIndexOf(lastSetRef.current);
				if (idx === -1) resetBrowse();
				else indexRef.current = idx;
			}, [history]);
			react.useEffect(function () {
				...
				var onKeyDown = function (e) {
					...
					if (up && !browsingRef.current) {
						...
						if (indexRef.current <= 0) {
							// 已到窗口最旧一条：若会话还有更早的历史未加载（分页窗口），
							// 触发补载并留在原地；补载后按文本重锚定，下一次 ↑ 自然进入更早区。
							if (hasMore && typeof loadHistoryOlder === "function") {
								e.preventDefault();
								e.stopPropagation();
								loadHistoryOlder();
							}
			...
			}, [input, history, inputActions, hasMore, loadHistoryOlder]);
			return null;
		}
		//#endregion
		//#region plugin
		function apply(ctx) {
			// 会话标准套件扩展：向每个 session 级 slot 注入 loadHistoryOlder —— 把更早的
			// 历史窗口拉进客户端快照（本插件的 state 即快照）；翻阅到最旧一条时自动分页，
			// 让 ↑ 能覆盖整段会话日志（压缩摘要之外）。
			if (ctx.sessions !== void 0 && typeof ctx.sessions.provide === "function") {
				ctx.sessions.provide({
					props: ["loadHistoryOlder"],
					resolve: function (binding) {
						return {
							props: {
								loadHistoryOlder: function () { void binding.session.loadOlder(); }
							}
						};
					}
				});
